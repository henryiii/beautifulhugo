...
- **Render hooks:** `layouts/_markup/render-image.html` sends Markdown images through the same image helpers as `beautifulfigure`. It emits a plain `<img>` (no `<figure>`, no PhotoSwipe), so goldmark block attributes still land on the `<p>` wrapper as before.
